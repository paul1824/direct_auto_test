...
class TestAddDbTables(BaseTestcase):
    ...
    def test_001(self):
        """添加自助数据集"""
        payload = '{"name":"gp_自助数据集_过滤","myAnalysis":false,"operators":[{"type":1,"value":[{"field":"gp[5f]合同事实表[' \
                  ...
                  '"id":"'+package_id+'"},"path":[],"commonTable":["gp_合同事实表_D"],' \
                  '"selectFieldId":"自助数据集_[6ce8][518c][65f6][95f4]","group":0}],"routersMap":{}},{"type":5,' \
                  '"customName":"过滤","comment":"","value":[{"id":"5e8cfea50d7a4979","filterValue":{"type":1,' \
                  '"assist":["购买合同","服务协议"],"value":["长期协议","长期协议订单"]},"fieldId":"自助数据集_[5408][540c][7c7b][578b]",' \
                  '"filterType":1}]}],"transferName":"gp_自助数据集_过滤","usedSpace":0,"editable":false,"selected":0,' \
        ...
        print(resp.text)
        self.assertEqual(GetDict(resp.text).getdict()['message'], 'success', msg='自助数据集字段设置有问题')